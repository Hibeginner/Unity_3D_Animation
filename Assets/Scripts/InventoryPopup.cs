...
public class InventoryPopup : MonoBehaviour {
    ...
    public void Refresh() {
        ...
        for (int i = 0; i < len; i++) {
            if (i < itemList.Count) {
                ...
                entry.callback.AddListener((BaseEventData data) => {
                    OnItem(item);//为每个物品触发不同的lambda函数
                });

                EventTrigger trigger = itemIcons[i].GetComponent<EventTrigger>();
                trigger.triggers.Clear();//清除监听器，以便从空白的状态刷新
                trigger.triggers.Add(entry);//将监听器函数添加到EventTrigger
            } else {
        ...
        if (_curItem == null) {//如果没有选择物品，则隐藏按钮
            ...
            if (_curItem == "health") {//仅使用按钮显示血量物品
